...
public class StabilityType implements PropertyBase {
    ...
    public void LoadProperty(Configuration config, String category) {
        ...
        EMP_Settings.stabilityTypes.put(name, new StabilityType(name, physEnable, supportDist, minFall, maxFall, canHang, holdOther));
    }
    ...
    public void GenDefaults() {
        ...
        if (!file.exists()) {
            try {
                ...
                EnviroMinePort.logger.log(Level.ERROR, "Failed to create file for StabilityTypes", e);
                return;
            }
        }
    ...
    public File GetDefaultFile() {
        return new File(EMP_ConfigHandler.loadedProfile + "StabilityTypes.cfg");
    }

    @Override
    public void generateEmpty(Configuration config, Object obj) {
        if (obj == null || !(obj instanceof String)) {
            EnviroMinePort.logger.log(Level.ERROR, "Tried to register config with non string object!", new Exception());
            return;
        }
    ...
    public void customLoad() {
        ...
        try {
            ...
            EnviroMinePort.logger.log(Level.ERROR, "Failed to load stability types!", e);
            return;
        }

        config.load();

        ArrayList<String> category = EMP_ConfigHandler.getSubCategories(config, this.categoryName());

        for (String s : category) {
            this.LoadProperty(config, s);
        }
